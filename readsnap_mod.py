...
def read_block(filename, block, parttype=-1, physical_velocities=True, arepo=0, no_masses=False, verbose=False):
    ...
    data_for_type = np.zeros(6,
                             bool)  # should be set to "True" below for the species for which data is stored in the data block
    ...
    # Initialize data array before the loop
    data = np.empty(allpartnum, dtype=dt)
    
    # Now the loop for multiple files
    for i in range(filenum):  # main loop over files
        ...
        if np.dtype(dt).itemsize * curpartnum != blocksize:
            print("something wrong with blocksize! expected =", np.dtype(dt).itemsize * curpartnum, "actual =",
                  blocksize)
            sys.exit()
